...
const Projects = ({ projectsData }: ProjectsProps) => {
    ...
    useEffect(() => {
        filterProjects(categories.includes('All') ? "All" : categories[0])
    }, [])
    ...
    return (
        <SectionWrapper id="projects" className="mx-4 md:mx-0 min-h-screen">
            ...
            <div className="md:w-full max-w-screen-sm mx-auto mt-6 flex justify-between items-center gap-2 md:gap-3 bg-white dark:bg-gray-800 p-2 rounded-md">
                {categories.map((c: string = "", i: number) => (
                    <span key={i} onClick={() => filterProjects(c)} className={`p-1.5 md:p-2 w-full text-sm md:text-base text-center capitalize rounded-md ${category.toLowerCase() === c.toLowerCase() ? "bg-violet-600 text-white" : "hover:bg-gray-100 hover:dark:bg-gray-900"} cursor-pointer transition-all`}>
                        {c}
                    </span>
                ))}
            </div>
...
export const ViewAll = ({ handleClick, title, scrollTo }: { handleClick: MouseEventHandler, title: string, scrollTo: string }) => {
    return (
        <>
            <div className="bg-white dark:bg-gray-900 w-4/5 mx-auto blur-xl z-20 -translate-y-14 h-16"></div>
